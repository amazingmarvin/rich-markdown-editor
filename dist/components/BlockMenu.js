...
const w3c_keyname_1 = require("w3c-keyname");
...
class BlockMenu extends React.Component {
    constructor() {
        ...
        this.handleKeyDown = (event) => {
            if (!this.props.isActive)
                return;
            let shortcut = event.key || (0, w3c_keyname_1.keyName)(event);
            if (event.shiftKey)
                shortcut = "⇧ + " + shortcut;
            if (event.altKey)
    ...
    get allItems() {
        ...
        let items = (0, block_1.default)(dictionary);
        const embedItems = [];
    ...
    get filtered() {
        const { search = "", uploadImage, canUploadImages, commands, } = this.props;
        const items = this.allItems;
        const filtered = items.filter(item => {
            ...
            if ((!uploadImage || !canUploadImages) && item.name === "image")
                return false;
            if (!search)
